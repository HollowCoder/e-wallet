...
from app.forms import RegistrationForm, LoginForm, AddCustomerForm, AddCardForm, PayBillForm, UserSettings
from app import app, db
from flask_login import current_user, login_user, login_required, logout_user
from app.models import User, Card, Settings
import simplify
import json
import requests
import time
...
def userSettings():
	form = UserSettings()
	dictSettings = {
			"emailNotifications" : form.emailNotifications.data,
			"smsNotifications" : form.smsNotifications.data
		}
	settings = Settings(emailNotifications=form.emailNotifications.data, smsNotifications=form.smsNotifications.data)
	if form.validate_on_submit():
		db.session.add(settings)
		db.session.commit()
		flash("Options Saved.")
		return render_template('userSettings.html', settings=settings, form=form, dictSettings=dictSettings)
	return render_template('userSettings.html', settings=settings, form=form, dictSettings=dictSettings)

@app.route('/scheduledPayments', methods=['GET', 'POST'])
def scheduledPayments():
	form = ScheduledPayments()
	

	return render_template('scheduledPayments.html')
